...
# Preprocess and create data functions
spk_utt_index = {}
...
def add_to_queue(unique_speakers, spk_utt_dict, candidates=c.CANDIDATES_PER_BATCH):
    data_produce_step = 0
    while True:
        if data_queue.full():
            sleep(0.1)
...
def best_batch(model, batch_size=c.BATCH_SIZE, candidates=c.CANDIDATES_PER_BATCH):
    ...
    get_batch_time = time() - orig_time

    orig_time = time()
    embeds = model.predict_on_batch(features)
    print("get batch time {0:.3}s, forward process time {1:.3}s".format(get_batch_time, time() - orig_time))
...
if __name__ == '__main__':
    model = rescnn_model()  # Replace with your actual model initialization
