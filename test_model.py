...
from eval.eval_metrics import evaluate
from src.models import rescnn_model
from src.triplet_loss import deep_speaker_loss
from src.utils import get_last_checkpoint_if_any, create_dir_and_delete_content
...
def eval_model(model, train_batch_size=c.BATCH_SIZE * c.TRIPLET_PER_BATCH, test_dir= c.TEST_DIR, check_partial=False, gru_model=None):
    ...
    fm, tpr, acc, eer, precision = evaluate(y_pred, y_true)
    return fm, tpr, acc, eer, precision
